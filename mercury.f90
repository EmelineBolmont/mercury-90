...
program mercury
  ...
  real(double_precision) :: cefac,time,tstop,dtout,h0,tol,en(3),am(3)
  character*8 id(NMAX)
  
  !------------------------------------------------------------------------------
  
  ! Get initial conditions and integration parameters
  call mio_in (time,tstop,dtout,h0,tol,rmax,rcen,jcen,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,id,&
  ...
  ! Main integration
  if (algor.eq.1) call mal_hcon (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,&
       rho,rceh,stat,id,ngf,opflag,ngflag,mdt_mvs,mco_h2mvs,mco_mvs2h)
  
  if (algor.eq.9) call mal_hcon (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,&
       rho,rceh,stat,id,ngf,opflag,ngflag,mdt_mvs,mco_iden,mco_iden)
  
  if (algor.eq.2) call mal_hvar (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,&
       rho,rceh,stat,id,ngf,opflag,ngflag,mdt_bs1)
  
  if (algor.eq.3) call mal_hvar (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,&
       rho,rceh,stat,id,ngf,opflag,ngflag,mdt_bs2)
  
  if (algor.eq.4) call mal_hvar (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,&
       rho,rceh,stat,id,ngf,opflag,ngflag,mdt_ra15)
  
  if (algor.eq.10) call mal_hcon (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,&
  ...
  call mio_dump (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,&
       id,ngf,epoch,opflag)
...
subroutine mio_in (time,tstop,dtout,h0,tol,rmax,rcen,jcen,en,am,cefac,ndump,nfun,nbod,nbig,m,x,v,s,rho,rceh,stat,&
  ...
  integer :: ndump,nfun
  real(double_precision) :: time,tstop,dtout,h0,tol,rmax,rcen,jcen(3)
...
subroutine mal_hvar (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,&
  ...
  real(double_precision) :: time,tstop,dtout,h0,tol,jcen(3),rcen,rmax
  ...
  call mce_init (h0,jcen,rcen,rmax,cefac,nbod,nbig,m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,outfile(2),1)
  ...
  do
     ...
     if (abs(tout-time).lt.abs(tsmall).and.opflag.ge.-1) then
        ...
        call mio_out (time,jcen,rcen,rmax,nbod,nbig,m,xh,vh,s,rho,stat,id,opflag,outfile(1))
        ...
        call mio_dump (time,tstop,dtout,h,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,&
             id,ngf,epoch,opflag)
        tdump = time
     end if
...
!~      call mco_iden (time,jcen,nbod,nbig,h,m,xh,vh,x0,v0,ngf,ngflag)
     ...
     if (nclo.gt.0.and.opflag.ge.-1) then
        itmp = 1
        if (nhit.ne.0) itmp = 0
        call mio_ce (time,rcen,rmax,nbod,nbig,m,stat,id,nclo,iclo,jclo,stopflag,tclo,dclo,ixvclo,jxvclo,nstored,itmp)
     ...
     if (nhit.gt.0) then
        ...
        dtflag = 1
        if (opflag.ge.0) opflag = 1
        call mce_init (h0,jcen,rcen,rmax,cefac,nbod,nbig,m,xh,vh,s,rho,rceh,rphys,rce,rcrit,id,outfile(2),0)
     end if
     ...
     if (abs(time-tfun).ge.abs(dtfun).and.opflag.ge.-1) then
        ...
        ! Check for ejections
        call mxx_ejec (time,rmax,en,am,jcen,2,nbod,nbig,m,xh,vh,s,stat,id,ejflag,outfile(3))
...
subroutine mal_hcon (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,stat,&
  ...
  do
     ...
     if (abs(tout-time).le.hby2.and.opflag.ge.-1) then
        ...
        call mio_dump (time,tstop,dtout,h0,tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,&
...
!~            call mco_iden (time,jcen,nbod,nbig,h0,m,x,v,xh0,vh0,ngf,ngflag)
        ...
        call onestep (time,h0,tol,rmax,en,am,jcen,rcen,nbod,nbig,m,x,v,s,rphys,rcrit,rce,stat,id,ngf,dtflag,&
...
!~            call mco_iden (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag)
        ...
        itmp = 2
        if ((algor.eq.11).or.(algor.eq.12)) itmp = 3
        call mxx_ejec (time,rmax,en,am,jcen,itmp,nbod,nbig,m,xh,vh,s,stat,id,ejflag,outfile(3))
